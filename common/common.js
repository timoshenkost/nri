 ...
 * поэтому команда отдыха доходит не до всех: закрытая вкладка её просто
 * не слышит, и заряды остаются потраченными. Чтобы отдых не терялся,
 * оболочка ставит метку времени, а страница при загрузке сверяет её со
 * своей и добирает пропущенное.
 * ------------------------------------------------------------------ */
const LongRest = {
  STAMP_KEY: 'long_rest_stamp',

  _seenKey(page) { return `long_rest_seen_${page}`; },

  _stamp() { return Number(Store.get(this.STAMP_KEY, 0)) || 0; },

  /* Отдых случился. Метку двигаем ДО рассылки команды, чтобы открытые
     вкладки записали себе уже новое значение. */
  mark() { Store.set(this.STAMP_KEY, Date.now()); },

  // Страница отработала отдых — запоминаем, на какой метке это было.
  seen(page) { Store.set(this._seenKey(page), this._stamp()); },

  /* Вызывается при загрузке страницы: если отдых был, пока вкладка
     стояла закрытой, восстановление отрабатывает сейчас. */
  catchUp(page, applyFn) {
    const missed = this._stamp() > (Number(Store.get(this._seenKey(page), 0)) || 0);
    if (missed) applyFn();
    this.seen(page);
    return missed;
  }
